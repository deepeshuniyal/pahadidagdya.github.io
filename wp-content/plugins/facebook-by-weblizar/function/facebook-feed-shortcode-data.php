        ...
        function NextProduct() {
		   ...
		   jQuery.ajax({
			...
			$('.weblizar-fb-product-popup-content .weblizar-fb-product-information .weblizar-comment').html($NewCurrent.find('dialog').html());
                ...
                if ($nextElm.length === 0) { $('.nav-btn.next').css({ 'display': 'none' }); }
                else { $('.nav-btn.next').css({ 'display': 'none' }); }
            }
...
jQuery(".facebook_feed_more_page").toggle(function()
{
    ...
    jQuery(this).text("See less..").siblings(".weblizar_fb_teaser").hide();	 
}, function(){
    jQuery(this).text("See more..").siblings(".weblizar_fb_complete").hide();
