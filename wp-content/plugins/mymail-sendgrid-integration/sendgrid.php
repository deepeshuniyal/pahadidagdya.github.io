...
define('MYMAIL_SENDGRID_VERSION', '0.4.5');
...
class MyMailSendGird{
	...
	public function verify($user = '', $pwd = '') {
		...
		$url = (!mymail_option(MYMAIL_SENDGRID_ID.'_secure') ? 'http' : 'https').'://api.sendgrid.com/api/profile.get.json';
	...
	public function do_call($path, $data = array(), $bodyonly = false, $timeout = 5) {
		...
		if($code != 200) return new WP_Error($code, $body->message);
